...
public class Original_DBSCAN {
    Map<Integer,List<Point>> clusters;// = new HashMap<Integer, List<Point>>();
    List<Point> points = new ArrayList<Point>();
    double eps;

    Original_DBSCAN(List<Point> pointList, double epsilon){
        this.eps = epsilon;
    ...
    public List<Point> getPointsList() throws IOException {
        ...
        while ((checkStr = buffrd.readLine()) != null && checkStr != "") {
            ...
            // double idx = Double.parseDouble(pt[0]);
    ...
    public Map<Integer, List<Point>> DBSCAN_CALL(int minPts) {
        int C = 0;                                     
        for(Point p: points) {
